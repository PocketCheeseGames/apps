...
@implementation SwapStress


- (SwapStress*)InitSwapStress
{
	[super InitPowerUp];
	...
	pPowerUpImage = 0;
	pPowerUpImage = [[DRResourceManager GetInstance] LoadTexture:@"PowerUpSwapStressMeter.png"];
...
- (bool)PlayerChanges:(Player*)pPlayer
{
	...
	if( [uf isCollision:pPlayer.xPos :pPlayer.yPos :xPos :yPos :offset] )
	{
		...
		[pPlayer AddPowerUp:PU_ITEM_SWAP_HP_METER];
		available = false;
