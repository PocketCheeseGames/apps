...
@implementation SwapLives


- (SwapLives*)InitSwapLives
{
	[super InitPowerUp];
	...
	pPowerUpImage = 0;
	pPowerUpImage = [[DRResourceManager GetInstance] LoadTexture:@"PowerUpSwapLives.png"];
...
- (bool)PlayerChanges:(Player*)pPlayer
{
	...
	if( [uf isCollision:pPlayer.xPos :pPlayer.yPos :xPos :yPos :offset] )
	{
		...
		[pPlayer AddPowerUp:PU_ITEM_SHIELD];
		available = false;
